...
public class v1_GPSSet
{
    ...
    public v1_GPSSet()
    {
    	...
    	_Dialog.ReSetSize(0.7f, 0.96f);
    	//_Dialog.findViewById(R.id.Close_GPSSET).setOnClickListener(new ViewClick());
        
    	//设置标题
    	_Dialog.SetCaption("GPS "+Tools.ToLocale("信息【GPS】"));
        ...
        _bindCompass.put("View", _Dialog.findViewById(R.id.iv_compassarrow));
        _bindCompass.put("ICallback", new ICallback(){
			@Override
			public void OnClick(String Str, Object ExtraStr) {
				Tools.SetTextViewValueOnID(_Dialog, R.id.tv_compassangle, ExtraStr+"");
			}});
...
//	private NEMALocate m_NEMALocate = new NEMALocate();
    ...
    private void CreateLayerOut()
    {
        ...
        if (m_Bitmap2==null)m_Bitmap2= Bitmap.createBitmap(LL.getMeasuredWidth(),LL.getMeasuredHeight(),Config.ARGB_8888);
...
//	        	 if (m_GPSInfoType.equals("系统"))
				...
				if (PubVar.m_GPSLocate.m_NEMALocate.GetUseNEMA())
				{
	        	 ...
	        	 _Dialog.SetCaption(String.format("GPS 信息【%1$s】%2$s", GpsAllType,JSType));
...
//	        	 _Dialog.SetCaption(String.format("GPS 信息【%1$s】", GpsAllType));
//	        	 
//				Refresh();
//			}});
    }
    ...
    {
        Tools.SetTextViewValueOnID(_Dialog, R.id.tv_sd, Tools.ToLocale("速度")+"："+Speed+ " km/h");
        Tools.SetTextViewValueOnID(_Dialog, R.id.tv_gc, Tools.ToLocale("高程")+"："+Elev+ " "+Tools.ToLocale("米"));
        
        String JDUnit = "精度："+Prec+" 米";
        if (PubVar.m_GPSLocate.m_NEMALocate.GetUseNEMA())JDUnit = "PDOP："+Prec;

        Tools.SetTextViewValueOnID(_Dialog, R.id.tv_pos_jd, JDUnit);
        Tools.SetTextViewValueOnID(_Dialog, R.id.tv_pos_zt, Tools.ToLocale("状态")+"："+Tools.ToLocale(Status));
        Tools.SetTextViewValueOnID(_Dialog, R.id.tv_jd, Tools.ToLocale("经度")+"："+JD);
        Tools.SetTextViewValueOnID(_Dialog, R.id.tv_wd, Tools.ToLocale("纬度")+"："+WD);
    }

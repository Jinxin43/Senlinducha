	...
	public Coordinate XYToWGS84(double x, double y, double z) {
		...
		if (this._CoorSystem.GetName().equals("WGS-84坐标")) {
			...
			if (this._CoorSystem.GetCoorTransMethod() == lkCoorTransMethod.enServenPara) {
				...
				// //比例因子(ppm->10e-6)
				// double a2 = k *
				// this._CoorSystem.GetTransToP74()/3600*Math.PI/180; //X旋转
				// (秒->弧度)
				// double a3 = k *
				// this._CoorSystem.GetTransToP75()/3600*Math.PI/180; //Y旋转
				// double a4 = k *
				// this._CoorSystem.GetTransToP76()/3600*Math.PI/180; //Z旋转

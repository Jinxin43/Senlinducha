...
//		_ExtendForView = PubVar.m_Workspace.GetDataSourceByEditing().GetEnvelope();
		...
		// this.getViewConvert().getSize().getWidth(); //缩放高度
		// else W = H*this.getViewConvert().getSize().getWidth() /
		// this.getViewConvert().getSize().getHeight(); //缩放宽度
		// return new
	...
	public Envelope AdjustEnvelopeFitScreen(Envelope pEnv) {
		...
		if (W >= H)
			H = W * this.getViewConvert().getSize().getHeight() / this.getViewConvert().getSize().getWidth(); // 缩放高度
		else
			W = H * this.getViewConvert().getSize().getWidth() / this.getViewConvert().getSize().getHeight(); // 缩放宽度
	...
	public Map(MapControl mapControl) {
		...
		} catch (Exception ex)// 防止WGS84ToXY为空
		{

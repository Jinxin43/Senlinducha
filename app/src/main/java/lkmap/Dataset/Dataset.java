...
public class Dataset {
	...
	/**
	 * 清理数据集，1-清理打上删除标识的实体(SYS_STATUS=1)， 2-处理新增且意外停止的实体(SYS_STATUS=2)，
	 * 
	 * @return
	 */
...
//					Log.e("SYS_GEO", bytes.length+"");
			...
			lkmap.Tools.Tools
					.ShowMessageBox("图层：" + this.getDataTableName() + "，查询数据失败！" + SYSID + SQL + " " + e.getMessage());
		} finally {
			if (DR != null) {
				DR.Close();
			}
	...
	public boolean AddGeometryOutEnvelope(List<Integer> idList) {
		...
		try {
			...
			lkmap.Tools.Tools.ShowMessageBox("图层：" + this.getDataTableName() + "，查询数据失败！" + SYSID + ex.getMessage());
		} finally {
			if (DR != null) {
				DR.Close();
			}

...
public class LayerManger {
	...
	public LayerManger() {
		...
		dialogView.SetButtonInfo("1," + R.drawable.icon_title_comfirm + "," + Tools.ToLocale("确定") + "  ,确定",
				btnCallback);
		// dialogView.SetButtonInfo("2,"+R.drawable.v1_newlayer_shp+","+Tools.ToLocale("导入图层")+"
		// ,导入图层", btnCallback);
		// dialogView.SetButtonInfo("3,"+R.drawable.v1_newlayer+","+Tools.ToLocale("新建图层")+"
		// ,新建图层", btnCallback);
	...
	private void DoCommand(String strCommond) {
		...
		if (strCommond.equals("导入图层")) {
			...
			sd.SetCallback(new ICallback() {
				...
				public void OnClick(String Str, final Object ExtraStrT) {
					...
					// Tools.ShowMessageBox(dialogView.getContext(),"开始时间："+StartTime+"\r\n结束时间："+EndTime);
		...
		if (strCommond.equals("属性")) {
			...
			// {
			// m_SelectLayer.SetLayerProjectType("自定义工程");
			// }
		...
		if (strCommond.equals("向上") || strCommond.equals("向下")) {
			...
			// 最上、最下了
			if (idx == 0 && strCommond.toString().equals("向上")) {
				Tools.ShowToast(dialogView.getContext(), "已经在最上层！");
				return;
			}
			if (idx == this.layerItemList.size() - 1 && strCommond.toString().equals("向下")) {
				Tools.ShowToast(dialogView.getContext(), "已经在最下层！");
				return;
			}
			if (strCommond.toString().equals("向上"))
				idx--;
			if (strCommond.toString().equals("向下"))
				idx++;
		...
		if (strCommond.equals("删除")) {
			Tools.ShowYesNoMessage(dialogView.getContext(),
					Tools.ToLocale("是否删除图层") + "【" + this.m_SelectLayer.GetLayerAliasName() + "】？", new ICallback() {
		...
		if (strCommond.equals("存模板")) {
			...
			if (pLayerList.size() == 0) {
				Tools.ShowMessageBox(dialogView.getContext(), "图层列表数量为0，无法保存模板！");
				return;
			}
	...
	private void LoadLayerInfo() {
		...
		for (v1_Layer lyr : this.copiedLayers) {
			...
			if (lyr.GetLayerTypeName().equals("点"))
				resourceId = R.drawable.v1_layertype_point;
			if (lyr.GetLayerTypeName().equals("线"))
				resourceId = R.drawable.v1_layertype_line;
			if (lyr.GetLayerTypeName().equals("面"))
			...
			hm.put("D5", lyr.GetSelectable()); // 是否可选择
		...
		for (int i = layerCount - 1; i >= 0; i--) {
			...
			if (layer.GetEditMode() == lkEditMode.enDelete) {
				...
				if (PubVar.m_DoEvent.m_ProjectDB.GetSQLiteDatabase().ExcuteSQL(delSql)) {
					...
					Tools.ShowMessageBox(this.dialogView.getContext(), "删除图层失败！");
				}
			}
		}

...
public class DataDict 
{
	...
	public DataDict()
	{
    	...
    	DialogView.SetOtherView(R.layout.d_dicdata);
    	DialogView.ReSetSize(0.56f,0.9f);
    	DialogView.SetButtonInfo("1,"+R.drawable.icon_title_comfirm+","+Tools.ToLocale("确定")+"  ,确定", btnCallback);
    	DialogView.SetButtonInfo("2,"+R.drawable.icon_d_property+","+Tools.ToLocale("行政区划管理")+"  ,行政区划管理", btnCallback);
    	...
    	String strqs ="林业";
		ArrayAdapter<String> quanshuAdapter = new ArrayAdapter<String>(DialogView.getContext(),
																	android.R.layout.simple_spinner_item,
																	strqs.split(","));
		((Spinner)DialogView.findViewById(R.id.sp_hangye)).setAdapter(quanshuAdapter);
		
		String strproject ="自定义工程,退耕还林,林地变更调查";
		...
		String strYinzi ="小班因子表,森林资源调查因子,林地变更调查因子";
		ArrayAdapter<String> yinZiAdapter = new ArrayAdapter<String>(DialogView.getContext(),
																	android.R.layout.simple_spinner_item,
																	strYinzi.split(","));
		Spinner spYinzi = ((Spinner)DialogView.findViewById(R.id.sp_yinzi));
		spYinzi.setAdapter(yinZiAdapter);
		spYinzi.setOnItemSelectedListener(new YinziOnItemSelectedListener());
	}
	    	...
	    	public void onClick(View arg0)
	    	{
	    		...
	    		if(Tag.equals("保存"))
	    		{
	    			if(isAdd)
	    			{
	    				if(mDictDB.addDict(Tools.GetTextValueOnID(DialogView,  R.id.et_name), 
	    						...
	    						parentID+"",
	    						Tools.GetSpinnerValueOnID(DialogView, R.id.sp_hangye), 
	    		...
	    		if(Tag.equals("删除"))
	    		{
	    			...
	    			else
	    			{
	    				...
	    				{
	    					Tools.ShowMessageBox("当前字典对象删除成功！");
	    					parentID = 0;
	    					ID = 0;
	    					clearEditText();
	...
	private void importDict(final String importFile)
	{
		try
		{
				...
				for(int i=1;i<row;i++)
				{
					...
					Cell CellTinzi= sheet.getCell(5,i);
					int pID = 0;
					...
					hm.put("yinzi", CellTinzi.getContents());
	...
	private void exportDict()
	{
		try
		{
	        ...
	        Label label6 = new Label(5, 0, "调查因子");
	        ...
	        for(HashMap<String,String> hm:allXZQH)
	        {
		        ...
		        Label dlabel6 = new Label(5, i, hm.get("YinZi"));
		        sheet.addCell(dlabel6);
		        i++;
	        }
	        book.write();
	        book.close();
	...
	{
		mDatas2.clear();
		List<HashMap<String, Object>> source = mDictDB.getDictData(Tools.GetSpinnerValueOnID(DialogView, R.id.sp_projectType), 
																   Tools.GetSpinnerValueOnID(DialogView, R.id.sp_hangye));
